...
# v4: Add multi-head self-attention. Add ffwd
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self, vocab_size):
        ...
        # self-attention head
        # because we have 4 heads (communication channels), we want 8 dimensional self attentions
        # so that when we concatenate these 4 self-attentions, we get 32 dimensional output
    ...
    def forward(self, idx, targets=None):
        ...
        x = self.sa_heads(x) # apply 4-head of self-attention. (B, T, C)
        x = self.ffwd(x)
